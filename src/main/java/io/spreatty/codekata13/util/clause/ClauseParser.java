...
public class ClauseParser<T> implements Iterator<Parenthesis<T>> {
    ...
    public ClauseParser(String source, List<Clause<T>> clauses) {
    ...
    public Parenthesis<T> next() {
        ...
        do {
            ...
            if (closingPosition == -1) {
                token = source.substring(position);
                position = source.length();
                break;
            }

            token = source.substring(position, closingPosition);
            position = closingPosition + opening.clause.getCloseString().length();
            closingFound = opening.clause.getTokenTester() == null
                    || opening.clause.getTokenTester().test(token);
        } while (!closingFound);
